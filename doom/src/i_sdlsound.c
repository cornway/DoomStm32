...
static allocated_sound_t *AllocateSound(sfxinfo_t *sfxinfo, size_t len)
{
    ...
    do
    {
        snd = Z_Malloc(sizeof(allocated_sound_t) + len, PU_STATIC, 0);
...
//    uint32_t alen;
    ...
    data_in = Z_Malloc(length * sizeof(float));
    src_data.data_in = data_in;
    src_data.src_ratio = (double)mixer_freq / samplerate;

    // We include some extra space here in case of rounding-up.
    src_data.output_frames = src_data.src_ratio * length + (mixer_freq / 4);
    src_data.data_out = Z_Malloc(src_data.output_frames * sizeof(float));
...
int expand_native_sound(sfxinfo_t *sfxinfo,
                                   byte *_data,
                                   int length)
{
    allocated_sound_t *snd;
    Mix_Chunk *chunk;
    Sint16 *expanded, *data;
    int i;

    length -= sizeof(wave_t);
    snd = AllocateSound(sfxinfo, length);
    length = length / 2;
    ...
    data = (Sint16 *)(_data + sizeof(wave_t));
    for (i=0; i < length; ++i)
    {
        expanded[i] = data[i];
    }
...
static boolean CacheSFX(sfxinfo_t *sfxinfo)
{
    ...
    if (!ExpandSoundData_SDL(sfxinfo, data + 8, samplerate, length))
    {
        return false;
    }
...
static void I_SDL_UpdateSoundParams(int handle, int vol, int sep)
{
    ...
    audio_set_pan(handle, left, right);
}
...
static boolean I_SDL_SoundIsPlaying(int handle)
{
    ...
    return audio_is_playing(handle);
}
