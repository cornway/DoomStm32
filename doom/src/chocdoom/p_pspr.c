...
{	
    ...
    psp->sy = WEAPONTOP - player->lookdir + FixedMul (player->bob, finesine[angle]);
}
...
{	
    ...
    // Is already down.
    if (psp->sy < WEAPONBOTTOM  - player->lookdir)
	return;
  ...
  pspdef_t*	psp )
{
    ...
    if (psp->sy > WEAPONTOP - player->lookdir)
	return;
    
    psp->sy = WEAPONTOP - player->lookdir;
...
void P_BulletSlope (mobj_t*	mo)
{
    ...
    {
        an += 2 << 26;
        bulletslope = (mo->player->lookdir) / 173;
    }
    }
}
