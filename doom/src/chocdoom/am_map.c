...
static pix_t*	fb; 			// pseudo-frame buffer
static int 	amclock;
...
{
    memset(fb, color, (f_w * f_h * sizeof(pix_t)));
}
...
#define PUTDOT(xx,yy,cc) fb[(yy)*f_w+(xx)]=pixel(cc)
...
void AM_drawMarks(void)
{
	...
	if (markpoints[i].x != -1)
	{
	    //      w = READ_LE_I16(marknums[i]->width);
	    //      h = READ_LE_I16(marknums[i]->height);
