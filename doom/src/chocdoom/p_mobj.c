...
void P_SpawnMapThing (mapthing_t* mthing)
{
	...
	d_memcpy(&playerstarts[READ_LE_I16(mthing->type)-1], mthing, sizeof(*mthing));
	if (!deathmatch)
  ...
  int		damage )
{
    ...
    if (damage < 6) {
        ...
        for (int i = 0; i < damage; i++) {
            rand = P_Random();
            rand = rand & 0x10 * (rand < 0 ? -1 : 1);
            th = P_SpawnMobj (x + rand,y - rand,z + rand, MT_BLOOD);
            th->momz = FRACUNIT*2;
            th->tics = P_Random()&0x7f + 0x800;
  ...
  mobjtype_t	type )
{
	...
	{
	    an = source->angle;
	    slope = (source->player->lookdir) / 173;
	}
    }
