  ...
  mobj_t*	target )
{
    ...
    mo->flags |= flags;	// special versions of items
}
  ...
  int 		damage )
{
	     ...
	     && (P_Random ()&1) )
	{
	    ...
	    if (source->player->readyweapon == wp_pistol ||
            source->player->readyweapon == wp_chaingun)
            thrust *= 2;
        else if (source->player->readyweapon == wp_shotgun)
            thrust *= 4;
        else if (source->player->readyweapon == wp_supershotgun)
            thrust *= 16;
        else
            thrust *= 3;
    }
