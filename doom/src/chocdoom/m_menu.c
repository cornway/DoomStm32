...
menu_t  NewLvlDef =
{
    1,		// # of menu items
    &MainDef,		// previous menu
    NewLVLMenu,	// menuitem_t ->
    M_DrawNewLevel,	// drawing routine ->
    48,63,              // x,y
    0		// lastOn
};
...
void M_DrawReadThis1(void)
{
    ...
    V_DrawPatchDirect (0, 0, (patch_t *)W_CacheLumpName(lumpname, PU_CACHE));
...
void M_DrawReadThis2(void)
{
    ...
    V_DrawPatchDirect(0, 0, (patch_t *)W_CacheLumpName(DEH_String("HELP1"), PU_CACHE));
}
...
void M_DrawSound(void)
{
    V_DrawPatchDirect (60, 38, (patch_t *)W_CacheLumpName(DEH_String("M_SVOL"), PU_CACHE));
...
void M_DrawMainMenu(void)
{
    V_DrawPatchDirect(94, 2,
                      (patch_t *)W_CacheLumpName(DEH_String("M_DOOM"), PU_CACHE));
}
...
void M_DrawNewGame(void)
{
    V_DrawPatchDirect(96, 14, (patch_t *)W_CacheLumpName(DEH_String("M_NEWG"), PU_CACHE));
    V_DrawPatchDirect(54, 38, (patch_t *)W_CacheLumpName(DEH_String("M_SKILL"), PU_CACHE));
}
...
void M_DrawEpisode(void)
{
    V_DrawPatchDirect(54, 38, (patch_t *)W_CacheLumpName(DEH_String("M_EPISOD"), PU_CACHE));
}
...
void M_DrawOptions(void)
{
    V_DrawPatchDirect(108, 15, (patch_t *)W_CacheLumpName(DEH_String("M_OPTTTL"),
                                               PU_CACHE));
	
    V_DrawPatchDirect(OptionsDef.x + 175, OptionsDef.y + LINEHEIGHT * detail,
		      (patch_t *)W_CacheLumpName(DEH_String(detailNames[detailLevel]),
			              PU_CACHE));

    V_DrawPatchDirect(OptionsDef.x + 120, OptionsDef.y + LINEHEIGHT * messages,
                      (patch_t *)W_CacheLumpName(DEH_String(msgNames[showMessages]),
                                      PU_CACHE));
#if ORIGCODE
  ...
  int	thermDot )
{
    ...
    xx = x;
    V_DrawPatchDirect(xx, y, (patch_t *)W_CacheLumpName(DEH_String("M_THERML"), PU_CACHE));
    xx += 8;
    for (i=0;i<thermWidth;i++)
    {
	V_DrawPatchDirect(xx, y, (patch_t *)W_CacheLumpName(DEH_String("M_THERMM"), PU_CACHE));
	xx += 8;
    }
    V_DrawPatchDirect(xx, y, (patch_t *)W_CacheLumpName(DEH_String("M_THERMR"), PU_CACHE));

    V_DrawPatchDirect((x + 8) + thermDot * 8, y,
		      (patch_t *)W_CacheLumpName(DEH_String("M_THERMO"), PU_CACHE));
}
  ...
  int		item )
{
    V_DrawPatchDirect(menu->x - 10, menu->y + item * LINEHEIGHT - 1, 
                      (patch_t *)W_CacheLumpName(DEH_String("M_CELL1"), PU_CACHE));
}
...
{
    V_DrawPatchDirect(menu->x - 10, menu->y + item * LINEHEIGHT - 1,
                      (patch_t *)W_CacheLumpName(DEH_String("M_CELL2"), PU_CACHE));
}
...
int M_StringHeight(char* string)
{
    size_t             i;
    int             h;
    int             height = READ_LE_I16(hu_font[0]->height);
...
boolean M_Responder (event_t* ev)
{
	    ...
	    currentMenu->menuitems[itemOn].status)
	{
	    ...
	    {
		currentMenu->menuitems[itemOn].routine(key_menu_forward);
		S_StartSound(NULL,sfx_stnmov);
	    }
	    else
	    {
...
void M_Drawer (void)
{
	...
	if (name[0])
	{
	    V_DrawPatchDirect (x, y, (patch_t *)W_CacheLumpName(name, PU_CACHE));
	}
	y += LINEHEIGHT;
    }
