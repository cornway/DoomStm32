...
extern int maxframe;
extern int vis_sprite_count;

#define vis_hack 0


#if vis_hack
#define vis_foreach(head, cur) \
for (vissprite_t *cur = (head),\
    *__next = cur->next;               \
     cur;                              \
     cur = __next,                     \
     __next = __next->next)
...
vissprite_t *vsprsortedtail;

//
// R_SortVisSprites
//
static void
link_vissprite (vissprite_t *vis)
{
    if (vsprsortedhead == NULL) {
        vsprsortedhead = vis;
        vsprsortedtail = vis;
        vis->next = NULL;
        vis->prev = NULL;
        ...
        vis_foreach(vsprsortedhead, cur) {
            if (vis->scale <= cur->scale) {
                if (cur->prev != NULL) {
                    cur->prev->next = vis;
                    vsprsortedhead = vis;
                }
    ...
    vis->prev = vsprsortedtail;
    vis->next = NULL;
    vsprsortedtail->next = vis;
    vsprsortedtail = vis;
}
...
void R_ProjectSprite (mobj_t* thing)
{
    ...
    iscale = FixedMul( FixedMul(FRACUNIT, tz), projection_n);
...
void R_AddSprites (sector_t* sec)
{
    ...
    lightnum = (sec->lightlevel >> LIGHTSEGSHIFT)+extralight + 1;
